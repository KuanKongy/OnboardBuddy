...
export function inferNodeType(filePath: string, exportedSymbols: string[]): NodeTypeInfo {
  ...
  if (p.includes(".test.") || p.includes(".spec.") || p.includes("/test") || p.includes("/__tests__")) {
    return { type: "TEST", colorClasses: "bg-purple-500/20 text-purple-400 border-purple-500/40", description: "Test suite" };
  }
  if (p.includes("/utils/") || p.includes("/helpers/") || p.includes("/util/")) {
    return { type: "UTIL", colorClasses: "bg-cyan-500/20 text-cyan-400 border-cyan-500/40", description: "Utility functions" };
  }
  if (p.includes("/routes/") || p.includes("/router") || p.includes("/api/")) {
    return { type: "API", colorClasses: "bg-green-500/20 text-green-400 border-green-500/40", description: "Express routing layer" };
  }
  if (p.includes("/services/") || p.includes("service")) {
    return { type: "SERVICE", colorClasses: "bg-blue-500/20 text-blue-400 border-blue-500/40", description: "Business logic service" };
  }
  if (p.includes("/middleware/") || p.includes("middleware")) {
    return { type: "MIDDLEWARE", colorClasses: "bg-amber-500/20 text-amber-400 border-amber-500/40", description: "Request middleware" };
  }
  if (p.includes("/db/") || p.includes("/database/") || p.includes("repositor")) {
    return { type: "DATA", colorClasses: "bg-orange-500/20 text-orange-400 border-orange-500/40", description: "Database layer" };
  }
  if (p.includes("config") || p.includes(".env") || p.includes("/env/")) {
    return { type: "ENV", colorClasses: "bg-yellow-500/20 text-yellow-400 border-yellow-500/40", description: "App configuration" };
  }
  ...
  return { type: "MODULE", colorClasses: "bg-slate-500/20 text-slate-400 border-slate-500/40", description: desc };
}
...
export function mockCoverage(nodeId: string): number {
  let hash = 0;
