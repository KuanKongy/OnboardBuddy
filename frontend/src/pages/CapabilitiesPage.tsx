...
export function CapabilitiesPage() {
  ...
  const drillInto = (nodeId: string): boolean => {
    // Select-first default (Settings → Viewing): every node is treated as a
    // leaf, so `GraphCanvas.activate` falls through to plain selection and the
    // side panel explains it; the panel's button carries the drill.
    if (!autoDrillEnabled("capabilities", id ?? "")) return false;
...
function CapabilityAside({ cap, busy, onDrill }: { cap: Capability; busy: boolean; onDrill: () => void }) {
  return (
    <aside className="graph-canvas !h-auto min-h-0 flex-1 overflow-y-auto !bg-card p-4 xl:flex-none xl:basis-[300px]">
      ...
      <Button size="xs" className="mt-4 w-full justify-center" onClick={onDrill} disabled={busy}>
        Drill into flows
  ...
  return (
    <div className="mb-2 shrink-0 rounded-md border border-border bg-card px-3 py-2 text-[0.75rem]">
      ...
      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-2 border-t border-border pt-2">
        ...
        {/* Width-capped, not flex-1: letting this column absorb the free
          ...
          <div className="min-w-[10rem]">
            ...
            <div className="flex flex-wrap gap-1.5">
              {cap.modules.map((mod) => {
                ...
                // No tooltip: the stored reason is the same membership
                // boilerplate for every chip, so it explained nothing.
                return <span key={mod.id}>{chip}</span>;
              })}
            </div>
          </div>
        )}
