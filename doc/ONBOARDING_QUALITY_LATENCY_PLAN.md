...
3. Overlays: role reading order + trust panel merge; retire role_path/doc_health prose specs.
4. Frontend: chaptered sidebar, suggested rail, TL;DR/diagram rendering.
5. Section cache + section critique.
6. Re-bench (fresh + re-run + concurrent) and content re-audit: golden-journey check (ONBOARDING_UX_GOALS.md) then the Diátaxis rubric (DIATAXIS_NOTES.md).
